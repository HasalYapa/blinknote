...
interface InputSectionProps {
  onSummarize: (input: string, inputType: string, summaryLength: string) => void;
  isLoading: boolean;
}

const InputSection: React.FC<InputSectionProps> = ({ onSummarize, isLoading }) => {
      ...
      <Tabs value={inputType} onValueChange={setInputType} className="w-full mb-6">
        ...
        <TabsContent value="text" className="mt-4">
          ...
          <p className="text-xs text-gray-400 mt-2">
            {textInput.length} characters 2 {textInput.split(' ').filter(word => word.length > 0).length} words
          </p>
        </TabsContent>
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-end">
        <div className="flex-1">
          ...
          <Select value={summaryLength} onValueChange={setSummaryLength}>
            ...
            <SelectContent>
              <SelectItem value="short">Short 2 Quick overview</SelectItem>
              <SelectItem value="medium">Medium 2 Balanced summary</SelectItem>
              <SelectItem value="detailed">Detailed 2 Comprehensive summary</SelectItem>
            </SelectContent>
          </Select>
        </div>
